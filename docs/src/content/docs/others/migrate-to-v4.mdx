...
-   rpcUrls: { [Chain.Ethereum]: "https://example.com" }
- });
...
SKConfig.setApiKey("swapKit", "your-swapdk-api-key");
...
SKConfig.setEnv("isStagenet", true);
SKConfig.setRpcUrl(Chain.THORChain, "https://example.com");
await swapDK.connectKeystore([Chain.THORChain], phrase);
const balance = await swapDK.getBalance(Chain.THORChain);
...
SKConfig.set({ rpcUrls: { [Chain.Ethereum]: "https://example.com" } });
