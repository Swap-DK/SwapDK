...
async function connectWallets() {
  ...
  await swapDK.connectKeystore([Chain.Bitcoin], "your secret phrase here");

  const connectedWallets = Object.keys(swapDK.getAllWallets());
}
```
...
async function executeSwap() {
  try {
    ...
    if (!assetValue.isGasAsset && selectedRoute.tx) {
      const isApproved = await swapDK.isAssetValueApproved(
        assetValue,
        selectedRoute.tx.from
      );

      if (!isApproved) {
        await swapDK.approveAssetValue(assetValue, selectedRoute.tx.from);
      }
    }
...
async function setupAndSwap() {
  ...
  await swapDK.connectKeystore([Chain.THORChain], "your secret phrase here");
...
async function estimateSwapFees() {
  try {
    ...
    const estimatedFee = await swapDK.estimateTransactionFee({
      type: "swap",
      feeOptionKey: FeeOption.Fast,
      params: {
        route,
...
async function advancedSwap() {
  try {
    const quoteResponse = await SwapKitApi.getSwapQuote({
      ...
      sourceAddress: swapDK.getAddress(Chain.Ethereum),
      destinationAddress: swapDK.getAddress(Chain.BinanceSmartChain),
