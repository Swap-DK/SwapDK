...
const balances = await Promise.all(
  chains.map((chain) => swapDK.getBalance(chain, true))
);
...
const mintQuote = await SwapKitApi.getSwapQuote({
  ...
  sellAmount: "10000000",
  buyAsset: "BTC/BTC",
  sourceAddress: swapDK.getAddress(Chain.Bitcoin),
  destinationAddress: swapDK.getAddress(Chain.THORChain),
  slippage: 3,
});

const mintTx = await swapDK.swap({
...
const synthSwapQuote = await SwapKitApi.getSwapQuote({
  ...
  sellAmount: "10000000",
  buyAsset: "ETH/ETH",
  sourceAddress: swapDK.getAddress(Chain.THORChain),
  destinationAddress: swapDK.getAddress(Chain.THORChain),
  slippage: 1,
});
...
const redeemQuote = await SwapKitApi.getSwapQuote({
  ...
  buyAsset: "ETH.ETH",
  sourceAddress: swapDK.getAddress(Chain.THORChain),
  destinationAddress: swapDK.getAddress(Chain.Ethereum),
  slippage: 3,
});

const redeemTx = await swapDK.swap({
...
await swapDK.swap({ route: modifiedRoute });
```
...
const tx = await swapDK.transfer({
  assetValue,
  recipient,
...
const result = await retry.executeWithRetry(async () => {
  return await swapDK.swap({ route });
});
```
...
- Check [Playgrounds](https://example.com)