---
title: Bun + React
description: A guide to integrate SwapDK with Bun and React applications.
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

This guide will help you integrate SwapDK into your Bun-powered React application. Bun is a fast JavaScript runtime that includes a bundler, test runner, and package manager.
...
```bash
bun create react my-swapdk-app
cd my-swapdk-app
```
...
Create your main React application with SwapDK:
...
export function App() {
  ...
  const connectWallet = async () => {
    try {
      ...
      setIsConnected(true);
      const ethAddress = await swapDK.getAddress(Chain.Ethereum);
  ...
  const disconnectWallet = () => {
    swapDK.disconnectWallet(Chain.Ethereum);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-4">SwapDK + Bun</h1>
      {!isConnected ? (
        <button
...
```bash
# .env
BUN_PUBLIC_SWAPDK_API_KEY=your_api_key_here
...
if (process.env.BUN_PUBLIC_SWAPDK_API_KEY) {
  SKConfig.setApiKey("swapKit", process.env.BUN_PUBLIC_SWAPDK_API_KEY);
}
```
...
For a complete working example, check out the [Bun + React playground](https://example.com)
