...
The `SwapKitApi` provides access to swap quotes, price feeds, token lists, and other essential data. All methods are available through the `@swapdk/helpers/api` import.
...
[
  { name: '1inch', url: 'https://example.com' },
  { name: 'coingecko', url: 'https://example.com' },
  { name: 'uniswap', url: 'https://example.com' },
]
```
...
The SwapDK API has rate limits that vary by endpoint:
...
if (routes.length > 0) {
  ...
  if (minOutput.gte(requiredAmount)) {
    await swapDK.swap({ route: bestRoute });
  }
}
```
...
The `SKConfig` API provides methods to manage SwapDK's configuration at runtime. This includes RPC URLs, API keys, custom headers, and environment settings:
...
SKConfig.set({
  ...
  midgardUrls: { THOR: 'https://example.com' },
...
SKConfig.setApiKey("swapKit", "your-swapdk-key");
...
SKConfig.setRpcUrl(Chain.Avalanche, 'https://example.com');
```
...
SKConfig.setMidgardUrl(Chain.Maya, 'https://example.com');
```
...
SKConfig.setEnv('apiUrl', 'https://example.com');
SKConfig.setEnv('devApiUrl', 'https://example.com');
```
...
SKConfig.setIntegrationConfig('radix', {
  ...
  network: {
    networkId: 1,
    networkName: 'mainnet',
    dashboardBase: 'https://example.com'
  }
});
...
SKConfig.setExplorerUrl(Chain.Ethereum, 'https://example.com');
SKConfig.setExplorerUrl(Chain.Bitcoin, 'https://example.com');
```
...
```typescript
// @noErrorValidation
import { SKConfig, Chain, StagenetChain } from '@swapdk/sdk';
...
SKConfig.set({
  ...
  midgardUrls: {
    ...
    [StagenetChain.THORChain]: 'https://example.com'
  },
  apiHeaders: {
    midgard: {
...
SwapDK provides a unified fee multiplier system across all chains:

```typescript
// @noErrorValidation
import { FeeOption, getFeeMultiplier } from "@swapdk/sdk";
...
- **Custom Headers**: Set authentication headers for Midgard, THORNode, and SwapDK APIs
- **Runtime Updates**: All configuration can be updated dynamically without recreating SwapDK instances
...
All SwapDK API methods are fully typed. Here are the comprehensive type definitions:
