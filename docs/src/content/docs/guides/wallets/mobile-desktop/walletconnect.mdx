...
1. Visit [WalletConnect Cloud](https://example.com)
...
async function connectMultipleChains() {
  ...
  try {
    await swapDK.connectWalletconnect(supportedChains);
...
async function connectWithAdvancedOptions() {
  ...
  try {
    await swapDK.connectWalletconnect([Chain.Ethereum], walletconnectOptions);
...
await swapDK.connectWalletconnect([Chain.Ethereum, Chain.Polygon, Chain.Arbitrum]);
...
async function sendOnArbitrum() {
  ...
  try {
    ...
    console.log("🔍 View on Arbiscan:", `https://example.com`
    return txHash;
  } catch (error) {
...
await swapDK.connectWalletconnect([Chain.Cosmos]);
...
await swapDK.connectWalletconnect([Chain.THORChain]);
...
async function sendTHORChain() {
  ...
  try {
    ...
    console.log("🔍 View on THORChain Explorer:", `https://example.com`
    return txHash;
  } catch (error) {
...
async function connectEVMChains() {
  ...
  try {
    await swapDK.connectWalletconnect(evmChains);
...
async function connectCosmosEcosystem() {
  ...
  try {
    await swapDK.connectWalletconnect(cosmosChains);
...
async function diagnosticConnection() {
  try {
    ...
    if (error.message.includes("project_id")) {
      ...
      console.log("2. Check your project configuration at https://cloud.walletconnect.com/");
...
export function WalletConnect({ onConnect, onError }: WalletConnectProps) {
  ...
  const connectWalletConnect = async () => {
    ...
    try {
      await swapDK.connectWalletconnect(selectedChains);
