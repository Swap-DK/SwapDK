...
- **MetaMask**: [Chrome Web Store](https://example.com)
- **Brave Wallet**: Built into Brave browser
- **Coinbase Wallet**: [Chrome Web Store](https://example.com)
- **Trust Wallet**: [Chrome Web Store](https://example.com)
### 2. Set Up Wallet
...
async function connectMultipleEVMChains() {
  ...
  try {
    await swapDK.connectEVMWallet(evmChains, WalletOption.METAMASK);
...
async function connectBraveWallet() {
  try {
    await swapDK.connectEVMWallet([Chain.Ethereum], WalletOption.BRAVE);
...
await swapDK.connectEVMWallet([Chain.Ethereum, Chain.Polygon, Chain.Arbitrum], WalletOption.METAMASK);
...
async function switchAndTransact() {
  try {
    ...
    await swapDK.connectEVMWallet([Chain.Polygon], WalletOption.METAMASK);
...
const secureSwapDK = SwapKit({
  config: {
    ...
    rpcUrls: {
      ...
      [Chain.Arbitrum]: "https://example.com"
    },
...
export function MetaMaskWallet({ onConnect, onError }: MetaMaskWalletProps) {
  ...
  const connectMetaMask = async () => {
    ...
    try {
      await swapDK.connectEVMWallet([selectedChain], WalletOption.METAMASK);
