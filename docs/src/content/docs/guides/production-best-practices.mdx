...
await swapDK.connectTrezor([Chain.Bitcoin]);
```
...
function validateSwapInput(params: any): void {
  if (!swapDK.validateAddress(params.recipient, params.chain)) {
...
const avaxBalance = await swapDK.getBalance(Chain.Avalanche);

const chains = [Chain.Ethereum, Chain.Bitcoin, Chain.Avalanche];
const balances = await Promise.all(
  chains.map((chain) => swapDK.getBalance(chain))
);
```
...
async function executeSwap(route: SwapRoute) {
  try {
    const txHash = await swapDK.swap({ route });
...
async function performSwap(params: SwapParams) {
  ...
  try {
    ...
    await swapDK.approveAssetValue(assetValue);

    setState({ status: "swapping", message: "Executing swap..." });
    const txHash = await swapDK.swap({ route: quote.routes[0] });
...
const txHash = await tracker.track("swap", () => swapDK.swap({ route }));
```
...
async function healthCheck(): Promise<HealthStatus> {
  const checks = await Promise.allSettled([
    ...
    swapDK.validateAddress("0x...", Chain.Ethereum),
...
- Review [Security considerations](https://example.com)
