...
const solWallet = await swapDK.getWallet(Chain.Solana);
...
await swapDK.connectKeystore([Chain.Solana], "your mnemonic phrase");

await swapDK.connectPhantom([Chain.Solana]);

await swapDK.connectSolflare([Chain.Solana]);

await swapDK.connectLedger([Chain.Solana]);
```
...
const tokenAccounts = await solToolbox.getTokenAccounts(
  swapDK.getAddress(Chain.Solana)
);
...
const createTokenAccount = async (mintAddress: string) => {
  const tokenAccountTx = await solToolbox.createAssociatedTokenAccount({
    mint: mintAddress,
    owner: swapDK.getAddress(Chain.Solana),
    payer: swapDK.getAddress(Chain.Solana),
  });
...
const jupiterSwap = await solToolbox.executeProgram({
  ...
  instruction: {
    keys: [
      ...
      {
        pubkey: swapDK.getAddress(Chain.Solana),
        isSigner: true,
        isWritable: false,
      },
    ],
...
const solToBtcQuote = await swapDK.getQuote({
  sellAsset: "SOL.SOL",
  sellAmount: "10",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Solana),
...
const solToStable = await swapDK.getQuote({
  sellAsset: "SOL.SOL",
  sellAmount: "5",
  buyAsset: "SOL.USDC-EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
  senderAddress: swapDK.getAddress(Chain.Solana),
  recipientAddress: swapDK.getAddress(Chain.Solana),
  providers: ["JUPITER"],
});
```
...
const jupiterAdvancedSwap = async () => {
  ...
  const swapTx = await solToolbox.executeJupiterSwap({
    quote,
    userPublicKey: swapDK.getAddress(Chain.Solana),
    priorityFee: 10000,
  });
...
const stakeSol = async () => {
  ...
  const stakeAccountTx = await solToolbox.createStakeAccount({
    ...
    authorizedStaker: swapDK.getAddress(Chain.Solana),
    authorizedWithdrawer: swapDK.getAddress(Chain.Solana),
  });
...
const withdrawStake = async (stakeAccount: string) => {
  ...
  setTimeout(async () => {
    const withdrawTx = await solToolbox.withdrawStake({
      stakeAccount,
      withdrawerPubkey: swapDK.getAddress(Chain.Solana),
...
const getNFTs = async () => {
  const nfts = await solToolbox.getNFTs(swapDK.getAddress(Chain.Solana));
...
const transferNFT = async (nftMint: string, recipient: string) => {
  const transferTx = await solToolbox.transferNFT({
    mint: nftMint,
    from: swapDK.getAddress(Chain.Solana),
    to: recipient,
    owner: swapDK.getAddress(Chain.Solana),
  });
...
const listNFT = async (nftMint: string, price: number) => {
  const listingTx = await solToolbox.createMagicEdenListing({
    mint: nftMint,
    seller: swapDK.getAddress(Chain.Solana),
    price: price * 1e9,
...
const customSolanaRpc = "https://example.com"
SKConfig.setRpcUrl(Chain.Solana, customSolanaRpc);


const heliusRpc = "https://example.com"
...
SKConfig.setRpcUrl(Chain.Solana, "https://example.com"
