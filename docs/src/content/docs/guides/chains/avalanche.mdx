...
const avaxWallet = await swapDK.getWallet(Chain.Avalanche);
...
await swapDK.connectKeystore([Chain.Avalanche], "your mnemonic phrase");

await swapDK.connectBrowserWallet(Chain.Avalanche);

await swapDK.connectWalletConnect([Chain.Avalanche]);

await swapDK.connectLedger([Chain.Avalanche]);
```
...
const wAvaxTransfer = await swapDK.transfer({
  recipient: "0x742c4B4F3e0b5b069F5DCF8A65Eaf8d3E888a3E7",
  assetValue: AssetValue.from({
    chain: Chain.Avalanche,
    symbol: "WAVAX",
...
const bridgeQuote = await swapDK.getQuote({
  ...
  recipientAddress: swapDK.getAddress(Chain.Avalanche),
});
...
const ethToAvaxBridge = await swapDK.getQuote({
  sellAsset: "ETH.ETH",
  sellAmount: "0.5",
  buyAsset: "AVAX.AVAX",
  senderAddress: swapDK.getAddress(Chain.Ethereum),
...
const avaxToBtcQuote = await swapDK.getQuote({
  sellAsset: "AVAX.AVAX",
  sellAmount: "10",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Avalanche),
...
const pngToRuneQuote = await swapDK.getQuote({
...
const pangolinQuote = await swapDK.getQuote({
...
const getPortfolioBalances = async () => {
  const balances = {};
  const address = swapDK.getAddress(Chain.Avalanche);
...
const pangolinStaking = await toolbox.call({
  ...
  abi: miniChefABI,
  funcName: "deposit",
  funcParams: [1, "5000000000000000000", swapDK.getAddress(Chain.Avalanche)],
});
```
...
const subnetInteraction = async () => {
  ...
  const subnetProvider = new JsonRpcProvider("https://example.com"
...
const avalancheProvider = new JsonRpcProvider("https://example.com"
...
const joeV2Trade = await toolbox.call({
  ...
  funcParams: [
    ...
    swapDK.getAddress(Chain.Avalanche),
    Date.now() + 600000,
  ],
  txOverrides: {
