...
const btcWallet = await swapDK.getWallet(Chain.Bitcoin);
...
await swapDK.connectKeystore([Chain.Bitcoin], "your mnemonic phrase");
...
const addresses = {
  nativeSegwit: swapDK.getAddress(Chain.Bitcoin),
...
const btcToEthQuote = await swapDK.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.1",
  buyAsset: "ETH.ETH",
...
const btcToUsdcQuote = await swapDK.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.05",
...
const btcToRuneQuote = await swapDK.getQuote({
  sellAsset: "BTC.BTC",
  sellAmount: "0.01",
  buyAsset: "THOR.RUNE",
  senderAddress: swapDK.getAddress(Chain.Bitcoin),
...
const customBitcoinRpc = "https://example.com"
...
SKConfig.setRpcUrl(Chain.Bitcoin, "https://example.com"
