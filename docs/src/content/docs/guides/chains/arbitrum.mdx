...
description: Complete guide to integrating Arbitrum One with SwapDK SDK for Layer 2 DeFi and cross-chain operations
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers Arbitrum One integration with SwapDK, including wallet connections, token transfers, cross-chain swaps, and Layer 2 specific optimizations.

## Overview

Arbitrum One is a leading Ethereum Layer 2 scaling solution that provides faster and cheaper transactions while maintaining Ethereum security. SwapDK provides full Arbitrum support through:
...
const arbWallet = await swapDK.getWallet(Chain.Arbitrum);
...
```typescript
// @noErrorValidation
import { getArbitrumToolbox } from '@swapdk/toolboxes/evm';
...
await swapDK.connectKeystore([Chain.Arbitrum], "your mnemonic phrase");

await swapDK.connectBrowserWallet(Chain.Arbitrum);

await swapDK.connectWalletConnect([Chain.Arbitrum]);

await swapDK.connectLedger([Chain.Arbitrum]);
```
...
const bridgeQuote = await swapDK.getQuote({
  ...
  recipientAddress: swapDK.getAddress(Chain.Arbitrum),
});
...
const usdcBridge = await swapDK.getQuote({
...
const arbToBtcQuote = await swapDK.getQuote({
  sellAsset: "ARB.ARB-0x912CE59144191C1204E64559FE8253a0e49E6548",
  sellAmount: "100",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Arbitrum),
...
const camelotQuote = await swapDK.getQuote({
  sellAsset: "ARB.ETH",
  sellAmount: "0.1",
  buyAsset: "ARB.USDC-0xaf88d065e77c8cC2239327C5EDb3A432268e5831",
  senderAddress: swapDK.getAddress(Chain.Arbitrum),
...
const traderjoeQuote = await swapDK.getQuote({
  sellAsset: "ARB.USDC-0xaf88d065e77c8cC2239327C5EDb3A432268e5831",
  sellAmount: "100",
...
const bestRateQuote = await swapDK.getQuote({
...
const arbBalance = await toolbox.getBalance(
  swapDK.getAddress(Chain.Arbitrum),
...
const arbitrumProvider = new JsonRpcProvider("https://example.com"
...
SKConfig.setRpcUrl(Chain.Arbitrum, "https://example.com"
...
4. **Optimize for Fast Settlement**:
   ...
   ```typescript
   const fastTx = await swapDK.transfer({
     recipient: "0x...",
