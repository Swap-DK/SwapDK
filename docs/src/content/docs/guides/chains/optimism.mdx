...
const opWallet = await swapDK.getWallet(Chain.Optimism);
...
await swapDK.connectKeystore([Chain.Optimism], "your mnemonic phrase");

await swapDK.connectBrowserWallet(Chain.Optimism);

await swapDK.connectWalletConnect([Chain.Optimism]);

await swapDK.connectLedger([Chain.Optimism]);
```
...
const opTransfer = await swapDK.transfer({
  recipient: "0x742c4B4F3e0b5b069F5DCF8A65Eaf8d3E888a3E7",
  assetValue: AssetValue.from({
    chain: Chain.Optimism,
    symbol: "OP",
...
const velodromeSwap = await toolbox.call({
  ...
  funcParams: [
    ...
    swapDK.getAddress(Chain.Optimism),
    Date.now() + 600000,
  ],
  txOverrides: {
...
const bridgeQuote = await swapDK.getQuote({
  ...
  recipientAddress: swapDK.getAddress(Chain.Optimism),
});
...
const ethToOpBridge = await swapDK.getQuote({
  sellAsset: "ETH.ETH",
  sellAmount: "0.5",
  buyAsset: "OP.ETH",
  senderAddress: swapDK.getAddress(Chain.Ethereum),
...
const opToBtcQuote = await swapDK.getQuote({
  sellAsset: "OP.OP-0x4200000000000000000000000000000000000042",
  sellAmount: "100",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Optimism),
...
const velodromeQuote = await swapDK.getQuote({
  sellAsset: "OP.ETH",
  sellAmount: "0.1",
  buyAsset: "OP.USDC-0x7F5c764cBc14f9669B88837ca1490cCa17c31607",
  senderAddress: swapDK.getAddress(Chain.Optimism),
...
const getOptimismPortfolio = async () => {
  const balances = {};
  const address = swapDK.getAddress(Chain.Optimism);
...
const claimRewards = await toolbox.call({
  ...
  abi: gaugeABI,
  funcName: "getReward",
  funcParams: [swapDK.getAddress(Chain.Optimism)],
});
```
...
const optimismProvider = new JsonRpcProvider("https://example.com"
...
SKConfig.setRpcUrl(Chain.Optimism, "https://example.com"
...
2. **Use Native Optimism Infrastructure**:
   ...
   ```typescript
   const veloQuote = await swapDK.getQuote({
     sellAsset: "OP.ETH",
     sellAmount: "1",
     buyAsset: "OP.USDC-0x7F5c764cBc14f9669B88837ca1490cCa17c31607",
     senderAddress: swapDK.getAddress(Chain.Optimism),
