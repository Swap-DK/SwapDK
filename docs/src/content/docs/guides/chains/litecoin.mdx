...
description: Complete guide to integrating Litecoin with SwapDK SDK for UTXO operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers Litecoin integration with SwapDK, including wallet connections, UTXO management, fast confirmations, and cross-chain swaps.

## Overview

Litecoin is a peer-to-peer cryptocurrency based on Bitcoin with faster block times and lower fees. SwapDK provides comprehensive Litecoin support through:
...
const ltcWallet = await swapDK.getWallet(Chain.Litecoin);
...
```typescript
// @noErrorValidation
import { getLitecoinToolbox } from '@swapdk/toolboxes/utxo';
...
await swapDK.connectKeystore([Chain.Litecoin], "your mnemonic phrase");

await swapDK.connectLedger([Chain.Litecoin]);

await swapDK.connectTrezor([Chain.Litecoin]);

await swapDK.connectKeepKey([Chain.Litecoin]);
```
...
const addresses = {
  nativeSegwit: swapDK.getAddress(Chain.Litecoin),
...
const ltcToBtcQuote = await swapDK.getQuote({
  sellAsset: "LTC.LTC",
  sellAmount: "1",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Litecoin),
...
const ltcToUsdcQuote = await swapDK.getQuote({
  sellAsset: "LTC.LTC",
  sellAmount: "2",
...
const ltcToRuneQuote = await swapDK.getQuote({
  sellAsset: "LTC.LTC",
  sellAmount: "0.5",
  buyAsset: "THOR.RUNE",
  senderAddress: swapDK.getAddress(Chain.Litecoin),
...
const fastArbitrage = async () => {
  const ltcToEthQuote = await swapDK.getQuote({
    sellAsset: "LTC.LTC",
    sellAmount: "5",
    buyAsset: "ETH.ETH",
    senderAddress: swapDK.getAddress(Chain.Litecoin),
...
const atomicSwapExample = async () => {
  const swapContract = {
    initiator: swapDK.getAddress(Chain.Litecoin),
...
const customLtcRpc = "https://example.com"
...
SKConfig.setRpcUrl(Chain.Litecoin, "https://example.com"
...
const consolidateUTXOs = async () => {
  const address = swapDK.getAddress(Chain.Litecoin);
...
```typescript
// @noErrorValidation
import { validateLitecoinAddress } from "@swapdk/toolboxes/utxo";
