...
const bchWallet = await swapDK.getWallet(Chain.BitcoinCash);
...
await swapDK.connectKeystore([Chain.BitcoinCash], "your mnemonic phrase");

await swapDK.connectLedger([Chain.BitcoinCash]);

await swapDK.connectTrezor([Chain.BitcoinCash]);
```
...
const addresses = {
  cashaddr: swapDK.getAddress(Chain.BitcoinCash),
...
const bchToBtcQuote = await swapDK.getQuote({
  sellAsset: "BCH.BCH",
  sellAmount: "1",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.BitcoinCash),
...
const bchToUsdtQuote = await swapDK.getQuote({
  sellAsset: "BCH.BCH",
  sellAmount: "0.5",
...
const bchToRuneQuote = await swapDK.getQuote({
  sellAsset: "BCH.BCH",
  sellAmount: "0.1",
  buyAsset: "THOR.RUNE",
  senderAddress: swapDK.getAddress(Chain.BitcoinCash),
...
const createPaymentRequest = async (amount: string, memo?: string) => {
  const paymentRequest = {
    ...
    outputs: [
      {
        address: swapDK.getAddress(Chain.BitcoinCash),
...
const customBchRpc = "https://example.com"
...
SKConfig.setRpcUrl(Chain.BitcoinCash, "https://example.com"
...
const manageUTXOs = async () => {
  const address = swapDK.getAddress(Chain.BitcoinCash);
