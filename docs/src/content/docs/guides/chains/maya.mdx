...
const mayaWallet = await swapDK.getWallet(Chain.Maya);
...
await swapDK.connectKeystore([Chain.Maya], "your mnemonic phrase");

await swapDK.connectKeplr([Chain.Maya]);

await swapDK.connectLedger([Chain.Maya]);
```
...
const mayaAddress = swapDK.getAddress(Chain.Maya);
...
const bondTx = await mayaToolbox.bondNode({
  ...
  bondAddress: swapDK.getAddress(Chain.Maya),
  gasLimit: 200000,
  gasPrice: "0.025cacao",
});
...
const withdrawLiquidity = async () => {
  const withdrawal = await swapDK.maya.withdraw({
    assetValue: AssetValue.from({ asset: "BTC.BTC", value: "0" }),
    percent: 50,
    from: "sym",
    to: "sym",
...
const privateSwap = async () => {
  const swapQuote = await swapDK.getQuote({
    sellAsset: "MAYA.CACAO",
    sellAmount: "100",
    buyAsset: "BTC.BTC",
    senderAddress: swapDK.getAddress(Chain.Maya),
  ...
  const privateSwapTx = await swapDK.swap({
...
const multiHopPrivateSwap = async () => {
  const multiHopQuote = await swapDK.getQuote({
    sellAsset: "ETH.USDC-0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
    sellAmount: "1000",
    buyAsset: "MAYA.CACAO",
    senderAddress: swapDK.getAddress(Chain.Ethereum),
    recipientAddress: swapDK.getAddress(Chain.Maya),
    providers: ["MAYA"],
    privacy: true,
  });

  const multiHopTx = await swapDK.swap({
...
const registerMayaName = async () => {
  ...
  const isAvailable = await swapDK.mayaNames.isAvailable(mayaName);
  ...
  const registerTx = await swapDK.maya.registerName({
    assetValue: registrationFee,
    name: mayaName,
    chain: "MAYA",
    address: swapDK.getAddress(Chain.Maya),
    owner: swapDK.getAddress(Chain.Maya),
...
const updateMayaName = async () => {
  const updateTx = await swapDK.maya.registerName({
...
const anonymousSwap = async () => {
  const anonymousQuote = await swapDK.getQuote({
    sellAsset: "BTC.BTC",
    sellAmount: "0.01",
    buyAsset: "MAYA.CACAO",
    senderAddress: swapDK.getAddress(Chain.Bitcoin),
    recipientAddress: swapDK.getAddress(Chain.Maya),
    providers: ["MAYA"],
    privacy: true,
    anonymous: true,
  ...
  const anonymousTx = await swapDK.swap({
...
const privateMultiAssetSwap = async () => {
  ...
  for (const { asset, amount } of assets) {
    ...
    const quote = await swapDK.getQuote({
      ...
      sellAmount: amount,
      buyAsset: "MAYA.CACAO",
      senderAddress: swapDK.getAddress(chain as Chain),
      recipientAddress: swapDK.getAddress(Chain.Maya),
      providers: ["MAYA"],
      privacy: true,
    });
...
const analyzePrivacyMetrics = async () => {
  const metrics = await swapDK.maya.getPrivacyMetrics();
  ...
  const personalScore = await swapDK.maya.getPersonalPrivacyScore(
...
const customMayaRpc = "https://example.com"
...
SKConfig.setRpcUrl(Chain.Maya, "https://example.com"
...
3. **Validate Privacy Requirements**:
   ...
   const validatePrivacySupport = async (
     ...
     const privacySupported = await swapDK.maya.checkPrivacySupport(
       sellAsset,
       buyAsset
     );
...
4. **Handle Privacy Delays**:
   ...
   const handlePrivacyDelays = async (txHash: string) => {
     ...
     const checkInterval = setInterval(async () => {
       const status = await swapDK.maya.getPrivacyTransactionStatus(txHash);
...
3. **Privacy feature unavailability**:
   ...
   const fallbackToRegularSwap = async (originalQuote: any) => {
     ...
     const regularQuote = await swapDK.getQuote({
       ...originalQuote,
       privacy: false,
       providers: ["THORCHAIN"],
     });
