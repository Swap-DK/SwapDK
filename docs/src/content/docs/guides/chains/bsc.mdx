...
description: Complete guide to integrating BNB Smart Chain with SwapDK SDK for DeFi operations and cross-chain swaps
---

import { Tabs, TabItem, Code } from "@astrojs/starlight/components";

This guide covers BNB Smart Chain (BSC) integration with SwapDK, including wallet connections, BEP-20 token transfers, cross-chain swaps, and BSC-specific DeFi protocols.

## Overview

BNB Smart Chain is a fast, low-cost blockchain compatible with Ethereum Virtual Machine. SwapDK provides comprehensive BSC support through:
...
const bscWallet = await swapDK.getWallet(Chain.BinanceSmartChain);
...
```typescript
// @noErrorValidation
import { getBSCToolbox } from '@swapdk/toolboxes/evm';
...
await swapDK.connectBrowserWallet(Chain.BinanceSmartChain);

await swapDK.connectWalletConnect([Chain.BinanceSmartChain]);

await swapDK.connectLedger([Chain.BinanceSmartChain]);
```
...
const cakeTransfer = await swapDK.transfer({
  recipient: "0x742c4B4F3e0b5b069F5DCF8A65Eaf8d3E888a3E7",
  assetValue: AssetValue.from({
    chain: Chain.BinanceSmartChain,
    symbol: "CAKE",
...
const pancakeswapTrade = await toolbox.call({
  ...
  funcParams: [
    ...
    swapDK.getAddress(Chain.BinanceSmartChain),
    Date.now() + 600000,
  ],
  txOverrides: {
...
const bridgeQuote = await swapDK.getQuote({
  ...
  recipientAddress: swapDK.getAddress(Chain.BinanceSmartChain),
});
...
const ethToBnbBridge = await swapDK.getQuote({
  sellAsset: "ETH.ETH",
  sellAmount: "0.5",
  buyAsset: "BSC.BNB",
  senderAddress: swapDK.getAddress(Chain.Ethereum),
...
const cakeToBtcQuote = await swapDK.getQuote({
  sellAsset: "BSC.CAKE-0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82",
  sellAmount: "100",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.BinanceSmartChain),
  recipientAddress: swapDK.getAddress(Chain.Bitcoin),
});

const crossChainSwap = await swapDK.swap({
  route: cakeToBtcQuote.routes[0],
  feeOptionKey: FeeOption.Fast,
});

const bnbToRuneQuote = await swapDK.getQuote({
  sellAsset: "BSC.BNB",
  sellAmount: "1",
  buyAsset: "THOR.RUNE",
  senderAddress: swapDK.getAddress(Chain.BinanceSmartChain),
...
const pancakeQuote = await swapDK.getQuote({
  sellAsset: "BSC.BNB",
  sellAmount: "1",
...
const pancakeSwap = await swapDK.swap({
  route: pancakeQuote.routes[0],
  feeOptionKey: FeeOption.Fast,
});

const complexSwapQuote = await swapDK.getQuote({
...
const getBSCPortfolio = async () => {
  const balances = {};
  const address = swapDK.getAddress(Chain.BinanceSmartChain);
...
const bscProvider = new JsonRpcProvider("https://example.com"
...
SKConfig.setRpcUrl(Chain.BinanceSmartChain, "https://example.com"
