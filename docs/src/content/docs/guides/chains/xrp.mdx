...
const xrpWallet = await swapDK.getWallet(Chain.Ripple);
...
await swapDK.connectKeystore([Chain.Ripple], "your mnemonic phrase");

await swapDK.connectXumm([Chain.Ripple]);

await swapDK.connectLedger([Chain.Ripple]);
```
...
const xrpAddress = swapDK.getAddress(Chain.Ripple);
...
const getTrustlines = async () => {
  const trustlines = await xrpToolbox.getTrustlines(
    swapDK.getAddress(Chain.Ripple)
  );
...
const finishEscrow = async (offerSequence: number) => {
  const finishTx = await xrpToolbox.finishEscrow({
    owner: swapDK.getAddress(Chain.Ripple),
    offerSequence,
  });
...
const xrpToBtcQuote = await swapDK.getQuote({
  sellAsset: "XRP.XRP",
  sellAmount: "1000",
  buyAsset: "BTC.BTC",
  senderAddress: swapDK.getAddress(Chain.Ripple),
...
const xrpToUsdcQuote = await swapDK.getQuote({
  sellAsset: "XRP.XRP",
  sellAmount: "500",
...
const multiSignTransaction = async (txBlob: string) => {
  ...
  return {
    txBlob,
    signature,
    account: swapDK.getAddress(Chain.Ripple),
  };
};
